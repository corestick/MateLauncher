...
public class LauncherModel {
	...
	synchronized boolean loadApplications(boolean isLaunching,
		...
		stopAndWaitForApplicationsLoader(); // 런처시작 처음에는 아무일도 안함
	...
	private synchronized void stopAndWaitForApplicationsLoader() {
		if (mApplicationsLoader != null && mApplicationsLoader.isRunning()) {
			...
			try {
				mApplicationsLoaderThread
						.join(APPLICATION_NOT_RESPONDING_TIMEOUT);
	...
	private static void updateApplicationInfoTitleAndIcon(
		...
		application.icon = Utilities.createIconThumbnail(
				info.activityInfo.loadIcon(manager), context);
	...
	private class ApplicationsLoader implements Runnable {
		...
		ApplicationsLoader(Launcher launcher, boolean isLaunching) {
			...
			mLauncher = new WeakReference<Launcher>(launcher); //메모리 관리에 관한
			mRunning = true;
			mId = sAppsLoaderCount.getAndIncrement();//스레드 관련 메소드
		}
			...
			if (apps != null && !mStopped) {
				...
				for (int i = 0; i < count && !mStopped; i++) {
					...
					ApplicationInfo application = makeAndCacheApplicationInfo(
							manager, appInfoCache, info, launcher);
	...
	private static void updateShortcutLabels(ContentResolver resolver,
			PackageManager manager) {
		final Cursor c = resolver.query(LauncherSettings.Favorites.CONTENT_URI,
				new String[] { LauncherSettings.Favorites._ID,
						LauncherSettings.Favorites.TITLE,
						LauncherSettings.Favorites.INTENT,
						LauncherSettings.Favorites.ITEM_TYPE }, null, null,
				null);
		...
		// if (changed) resolver.notifyChange(Settings.Favorites.CONTENT_URI,
		// null);
	}
				...
				while (!mStopped && c.moveToNext()) {
					try {
						...
						case LauncherSettings.Favorites.ITEM_TYPE_LIVE_FOLDER:
							...
							liveFolderInfo.uri = Uri.parse(c
									.getString(uriIndex));
							liveFolderInfo.baseIntent = intent;
							liveFolderInfo.displayMode = c
									.getInt(displayModeIndex);

							loadLiveFolderIcon(launcher, c, iconTypeIndex,
	...
	private static void loadLiveFolderIcon(Launcher launcher, Cursor c,
		...
		case LauncherSettings.Favorites.ICON_TYPE_RESOURCE:
			...
			try {
				...
				liveFolderInfo.icon = launcher.getResources().getDrawable(
	 ...
	 */
	private ApplicationInfo getApplicationInfoShortcut(Cursor c,
	...
	/**
	 * Remove an item from the in-memory represention of a user folder. Does not
	 * change the DB.
	 */
