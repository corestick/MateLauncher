	...
	public void selectApp() {
		...
		listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parentView, View view,
				...
				Toast.makeText(mLauncher, setupAppPacName.get(position), 30).show();
...
// 아바타 핸드폰 번호 매칭 
	...
	public void SetOn_off(ApplicationInfo info) {
		SetOn = new Button(mLauncher);
		SetOff = new Button(mLauncher);
		SetOn.setOnClickListener(this);
		addView(edit);
		addView(SetOff);

		LayoutParams param1 = (LayoutParams) SetOn.getLayoutParams();
		param1.width = 200;
		param1.height = 110;
		SetOn.setLayoutParams(param1);

		LayoutParams param2 = (LayoutParams) SetOff.getLayoutParams();
		param2.width = 200;
		param2.height = 110;
		SetOff.setLayoutParams(param1);

		SetOn.setText("말풍선ON");
		SetOff.setText("말풍선Off");
	}
	...
	public void onClick(View v) {
		if (v.equals(b)) {
			...
			CreateSendView();
			return;
		} else if (v.equals(SetOn)) {
			// TextView
			// mLauncher.getWorkspace().addView(child, width, height)
			return;
		} else if (v.equals(SetOff)) {
			return;
		}
	...
	public void loadApp() {
		...
		List appinfo = mLauncher.getPackageManager().getInstalledPackages(
				PackageManager.GET_INSTRUMENTATION | PackageManager.GET_UNINSTALLED_PACKAGES);
 ...
 //설치앱에서 커스텀 어뎁터
	public class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Drawable icon = Utilities.createIconThumbnail(setupAppIcon.get(position), getContext());
