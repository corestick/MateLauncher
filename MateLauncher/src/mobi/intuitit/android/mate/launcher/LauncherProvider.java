...
public class LauncherProvider extends ContentProvider {
	...
	static final String AUTHORITY = "mobi.intuitit.android.mate.launcher.settings";

	static final String EXTRA_BIND_SOURCES = "mobi.intuitit.android.mate.launcher.settings.bindsources";
	static final String EXTRA_BIND_TARGETS = "mobi.intuitit.android.mate.launcher.settings.bindtargets";
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE gestures (" + "_id INTEGER PRIMARY KEY,"
					...
					+ "iconType INTEGER," + "iconPackage TEXT,"
		...
		private boolean convertDatabase(SQLiteDatabase db) {
			...
			if (converted) {
				...
				if (LOGD)
					Log.d(LOG_TAG,
							"converted and now triggering widget upgrade");
				convertWidgets(db);
			}
		 ...
		 * equivalents. This method allocates appWidgetIds, and then hands off
		 * to LauncherAppWidgetBinder to finish the actual binding.
		 */
			...
			try {
				...
				while (c != null && c.moveToNext()) {
					...
					try {
						...
						if (LOGD)
							Log.d(LOG_TAG, "allocated appWidgetId="
									+ appWidgetId + " for favoriteId="
									+ favoriteId);
		 ...
		 */
		private void launchAppWidgetBinder(int[] bindSources,
				ArrayList<ComponentName> bindTargets) {
		...
		private boolean addShortcut(SQLiteDatabase db, ContentValues values,
			...
			values.put(Favorites.ICON_RESOURCE, mContext.getResources()
					.getResourceName(iconResId));
