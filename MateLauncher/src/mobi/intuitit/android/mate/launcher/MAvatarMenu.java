...
public class MAvatarMenu extends LinearLayout implements OnClickListener {
	...
	public void onClick(View v) {
		...
		if (v.equals(imgCall)) {
			...
			if (intent == null) {
				Toast.makeText(getContext(), "트위터앱을 설치하세요.", Toast.LENGTH_SHORT)
						.show();
			} else {
				mLauncher.startActivity(intent);
			}
			...
			if (intent == null) {
				Toast.makeText(getContext(), "카카오톡앱을 설치하세요.", Toast.LENGTH_SHORT)
						.show();
			} else {
				mLauncher.startActivity(intent);
			}
	...
	public void showMsgDialog(final String type) {
		...
		dig.setButton("전송", new DialogInterface.OnClickListener() {
