 ...
 */
public class MWorkspace extends WidgetSpace implements DropTarget, DragSource,
		DragScroller {
	 ...
	 */
	private MLayout.CellInfo mDragInfo;
	 ...
	 *            values.
	 */
	public MWorkspace(Context context, AttributeSet attrs) {
	 ...
	 *            Unused.
	 */
	public MWorkspace(Context context, AttributeSet attrs, int defStyle) {
	...
	Folder getOpenFolder() {
		MLayout currentScreen = (MLayout) getChildAt(mCurrentScreen);
	...
	ArrayList<Folder> getOpenFolders() {
		...
		for (int screen = 0; screen < screens; screen++) {
			MLayout currentScreen = (MLayout) getChildAt(screen);
			int count = currentScreen.getChildCount();
			for (int i = 0; i < count; i++) {
				View child = currentScreen.getChildAt(i);
				if (child instanceof Folder) {
	...
	void addInScreen(View child, int screen, int x, int y, int spanX,
		...
		final MLayout group = (MLayout) getChildAt(screen);
		MLayout.LayoutParams lp = (MLayout.LayoutParams) child
				.getLayoutParams();
		if (lp == null) {
			lp = new MLayout.LayoutParams(x, y);
		} else {
			lp.x = x;
			lp.y = y;
		}
	...
	void enableChildrenCache() {
		final int count = getChildCount();
		for (int i = 0; i < count; i++) {
			final MLayout layout = (MLayout) getChildAt(i);
	...
	void startDrag(MLayout.CellInfo cellInfo) {
		...
		MLayout current = ((MLayout) getChildAt(mCurrentScreen));
	...
	void addApplicationShortcut(ApplicationInfo info,
			MLayout.CellInfo cellInfo, boolean insertAtFirst) {

	}

	public void onDrop(DragSource source, int x, int y, int xOffset,
			int yOffset, Object dragInfo) {
		final MLayout mLayout = getCurrentDropLayout();

		if (source != this) {
			onDropExternal(x - xOffset, y - yOffset, dragInfo, mLayout);
		} else {
			// Move internally
			if (mDragInfo != null) {
				...
				if (index != mDragInfo.screen) {
					final MLayout originalCellLayout = (MLayout) getChildAt(mDragInfo.screen);
					originalCellLayout.removeView(cell);
					mLayout.addView(cell);
				}

				mLayout.onDropChild(cell, x - xOffset, y - yOffset);

				final ItemInfo info = (ItemInfo) cell.getTag();
				MLayout.LayoutParams lp = (MLayout.LayoutParams) cell
						.getLayoutParams();
	...
	private void onDropExternal(int x, int y, Object dragInfo, MLayout mLayout) {
		onDropExternal(x, y, dragInfo, mLayout, false);
	}
	...
	private void onDropExternal(int x, int y, Object dragInfo, MLayout mLayout,
		...
		Log.e("onDropExternal", "info.itemType>>>" + info.itemType);
		...
		case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
			...
			view = mLauncher.createShortcut(R.layout.application, mLayout,
		...
		mLayout.addView(view, insertAtFirst ? 0 : -1);
		view.setOnLongClickListener(mLongClickListener);

		mLayout.onDropChild(view, x, y);
		MLayout.LayoutParams lp = (MLayout.LayoutParams) view.getLayoutParams();
	...
	/**
	 * Return the current {@link MLayout}, correctly picking the destination
	 * screen while a scroll is in progress.
	 */
	private MLayout getCurrentDropLayout() {
	...
	public void onDropCompleted(View target, boolean success) {
		...
		if (success) {
			if (target != this && mDragInfo != null) {
				final MLayout mLayout = (MLayout) getChildAt(mDragInfo.screen);
				mLayout.removeView(mDragInfo.cell);
			...
			if (mDragInfo != null) {
				final MLayout mLayout = (MLayout) getChildAt(mDragInfo.screen);
				mLayout.onDropAborted(mDragInfo.cell);
			}
		}
	 ...
	 */
	private Search findSearchWidget(MLayout screen) {
